...
extends Module {
    ...
    public void onRender3D(Render3DEvent event) {
        if ((Display.isActive() || Display.isVisible()) && this.chams.getValue().booleanValue()) {
            if (EntityUtil.posEqualsBlock(PlayerUtil.getPlayerPos().up(), (Block)Blocks.WATER).booleanValue()) {
                return;
            }
            ...
            ((IEntityRenderer)ItemChams.mc.entityRenderer).invokeRenderHand(mc.getRenderPartialTicks(), 2);
            shader.stopDraw();
