...
extends Module {
    ...
    @Override
    public void onTick() {
        if (!(VoidView.fullNullCheck() || this.noEnd.getValue().booleanValue() && VoidView.mc.player.dimension == 1 || !this.timer.passedMs(this.updates.getValue().intValue()))) {
    ...
    public void onRender3D(Render3DEvent event) {
        if (VoidView.fullNullCheck() || this.noEnd.getValue().booleanValue() && VoidView.mc.player.dimension == 1) {
            return;
        }
    ...
    private List<BlockPos> findVoidHoles() {
        BlockPos playerPos = EntityUtil.getPlayerPos((EntityPlayer)VoidView.mc.player);
    ...
    private boolean isVoid(BlockPos pos) {
        return (VoidView.mc.world.getBlockState(pos).getBlock() == Blocks.AIR || this.air.getValue() == false && VoidView.mc.world.getBlockState(pos).getBlock() != Blocks.BEDROCK) && pos.getY() < 1 && pos.getY() >= 0;
    }
}
