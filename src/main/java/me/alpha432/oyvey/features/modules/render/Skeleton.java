    ...
    private void drawSkeleton(Render3DEvent event, EntityPlayer e) {
        ...
        if (entPos != null && e.isEntityAlive() && !e.isDead && e != Skeleton.mc.player && !e.isPlayerSleeping()) {
            ...
            if (OyVey.friendManager.isFriend(e.getName())) {
                ...
                GlStateManager.color((float)((float)ClickGui.getInstance().red.getValue().intValue() / 255.0f), (float)((float)ClickGui.getInstance().green.getValue().intValue() / 255.0f), (float)((float)ClickGui.getInstance().blue.getValue().intValue() / 255.0f), (float)this.alpha.getValue().intValue());
            }
