...
extends Module {
    ...
    public void onUpdate() {
        if (!CombatLogger.fullNullCheck()) {
            this.spots.removeIf(spot -> CombatLogger.mc.player.getDistanceSq((Entity)spot.getEntity()) >= MathUtil.square(this.range.getValue().floatValue()));
        }
    }
    ...
    public void onConnection(ConnectionEvent event) {
        if (event.getStage() == 0) {
            ...
            if (entity != null && this.message.getValue().booleanValue()) {
                Command3.sendMessage("\\u00A75" + entity.getName() + " just logged in" + (this.coords.getValue() != false ? " at (" + (int)entity.posX + ", " + (int)entity.posY + ", " + (int)entity.posZ + ")!" : "!"), this.notification.getValue());
            }
            ...
            if (this.message.getValue().booleanValue()) {
                Command3.sendMessage("\\u00A7d" + event.getName() + " just logged out" + (this.coords.getValue() != false ? " at (" + (int)entity.posX + ", " + (int)entity.posY + ", " + (int)entity.posZ + ")!" : "!"), this.notification.getValue());
            }
    ...
    private void renderNameTag(String name, double x, double yi, double z, float delta, double xPos, double yPos, double zPos) {
        ...
        GlStateManager.rotate((float)(-CombatLogger.mc.getRenderManager().playerViewY), (float)0.0f, (float)1.0f, (float)0.0f);
        GlStateManager.rotate((float)CombatLogger.mc.getRenderManager().playerViewX, (float)(CombatLogger.mc.gameSettings.thirdPersonView == 2 ? -1.0f : 1.0f), (float)0.0f, (float)0.0f);
