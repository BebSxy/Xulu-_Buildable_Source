...
extends Module {
    ...
    private void doVirtue(MoveEvent event) {
        if (this.mode.getValue() == Mode.VIRTUE && (LongJump.mc.player.moveForward != 0.0f || LongJump.mc.player.moveStrafing != 0.0f && !EntityUtilPhob.isOnLiquid() && !EntityUtilPhob.isInLiquid())) {
            if (this.stage == 0) {
