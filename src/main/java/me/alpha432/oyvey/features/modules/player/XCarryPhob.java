...
extends Module {
    ...
    public void onUpdate() {
        if (this.shiftClicker.getValue().booleanValue() && XCarryPhob.mc.currentScreen instanceof GuiInventory) {
            Slot slot;
            boolean ourBind;
            ...
            if ((Keyboard.isKeyDown((int)42) && this.withShift.getValue().booleanValue() || ourBind) && Mouse.isButtonDown((int)0) && (slot = ((GuiInventory)XCarryPhob.mc.currentScreen).getSlotUnderMouse()) != null && InventoryUtilPhob.getEmptyXCarry() != -1) {
        ...
        if (this.autoDuelOn) {
            ...
            if (this.autoDuelOn) {
                if (!this.obbySlotDone && !XCarryPhob.mc.player.inventory.getStackInSlot((int)(this.obbySlot.getValue().intValue() - 1)).isEmpty) {
                    this.addTasks(36 + this.obbySlot.getValue() - 1);
                }
                this.obbySlotDone = true;
                if (!this.slot1done && !((Slot)XCarryPhob.mc.player.inventoryContainer.inventorySlots.get((int)this.slot1.getValue().intValue())).getStack().isEmpty) {
                    this.addTasks(this.slot1.getValue());
                }
                this.slot1done = true;
                if (!this.slot2done && !((Slot)XCarryPhob.mc.player.inventoryContainer.inventorySlots.get((int)this.slot2.getValue().intValue())).getStack().isEmpty) {
                    this.addTasks(this.slot2.getValue());
                }
                this.slot2done = true;
                if (!this.slot3done && !((Slot)XCarryPhob.mc.player.inventoryContainer.inventorySlots.get((int)this.slot3.getValue().intValue())).getStack().isEmpty) {
    ...
    public void onDisable() {
        ...
        if (!this.simpleMode.getValue().booleanValue()) {
            ...
            XCarryPhob.mc.player.connection.sendPacket((Packet)new CPacketCloseWindow(XCarryPhob.mc.player.inventoryContainer.windowId));
        }
    }
