...
implements Util {
    ...
    public static double getBaseMoveSpeed() {
        ...
        if (EntityUtill.mc.player != null && EntityUtill.mc.player.isPotionActive(Potion.getPotionById((int)1))) {
            int amplifier = EntityUtill.mc.player.getActivePotionEffect(Potion.getPotionById((int)1)).getAmplifier();
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtill.isDead(entity) || entity.equals((Object)EntityUtill.mc.player) || entity instanceof EntityPlayer && OyVey.friendManager.isFriend(entity.getName()) || EntityUtill.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static double getMaxSpeed() {
        ...
        if (EntityUtill.mc.player.isPotionActive(Objects.requireNonNull(Potion.getPotionById((int)1)))) {
            maxModifier *= 1.0 + 0.2 * (double)(Objects.requireNonNull(EntityUtill.mc.player.getActivePotionEffect(Objects.requireNonNull(Potion.getPotionById((int)1)))).getAmplifier() + 1);
        }
        return maxModifier;
    }
    ...
    public static EntityPlayer getClosestEnemy(double distance) {
        ...
        for (EntityPlayer player : EntityUtill.mc.world.playerEntities) {
            ...
            if (!(EntityUtill.mc.player.getDistanceSq((Entity)player) < EntityUtill.mc.player.getDistanceSq((Entity)closest))) continue;
            closest = player;
        }
        return closest;
    }
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        for (EntityPlayer player : EntityUtill.mc.world.playerEntities) {
            if (EntityUtill.mc.player.getDistanceSq((Entity)player) > range * range || OyVey.friendManager.isFriend(player)) continue;
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtill.mc.world.playerEntities) {
            ...
            healthSB.append(hp);
            int distanceInt = (int)EntityUtill.mc.player.getDistance((Entity)player);
            ...
            output.put(healthSB.toString() + " " + (OyVey.friendManager.isFriend(player) ? "\u00a7b" : "\u00a7r") + player.getName() + " " + distanceSB.toString() + " \u00a7f" + OyVey.totemPopManager.getTotemPopString(player) + OyVey.potionManager.getTextRadarPotion(player), (int)EntityUtill.mc.player.getDistance((Entity)player));
