...
extends Module {
    ...
    public void onTick() {
        if (this.currentPos != null) {
            ...
            if (Speedmine.mc.player != null && this.silentSwitch.getValue().booleanValue() && this.timer.passedMs((int)(2200.0f * OyVey.serverManager.getTpsFactor()))) {
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        if (event.getStage() == 0) {
            ...
            if (this.noBreakAnim.getValue().booleanValue() && event.getPacket() instanceof CPacketPlayerDigging && (packet = (CPacketPlayerDigging)event.getPacket()) != null) {
                packet.getPosition();
                try {
                    for (Entity entity : Speedmine.mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(packet.getPosition()))) {
